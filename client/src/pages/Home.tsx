...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-20">
      <header className="pt-20 pb-12 px-6 sm:px-12 max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl sm:text-5xl font-display font-bold tracking-tight">Your Lists</h1>
          <p className="text-muted-foreground mt-3 text-lg">Curate what you consume next.</p>
        </div>
        <Button 
      ...
      <main className="px-6 sm:px-12 max-w-7xl mx-auto">
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-secondary/40 rounded-3xl h-48 animate-pulse" />
            ))}
          </div>
        ) : lists?.length === 0 ? (
          <div className="text-center py-32 bg-secondary/20 rounded-3xl border border-dashed border-border/50">
            ...
            <h2 className="text-2xl font-display font-medium mb-2">No lists yet</h2>
            <p className="text-muted-foreground mb-8 max-w-md mx-auto">Create your first list to start tracking movies, books, or anything else you want to get to.</p>
            <Button onClick={() => setIsCreateOpen(true)} variant="outline" className="rounded-xl h-12 px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <AnimatePresence>
              {lists?.map((list) => (
                ...
                >
                  <Link href={`/lists/${list.id}`} className="block h-full group">
                      ...
                      <div className="relative z-10 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto pt-6 flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-muted-foreground ml-2">
                            {list.items?.filter(i => i.isSeen).length || 0} / {list.items?.length || 0}
                          </span>
                        </div>
                      </div>
                    </div>
